...
contract OffchainOrderModule is IOffchainOrderModule, IMarketEvents, IAccountEvents {
    ...
    function settleOffchainOrder(
        ...
        if (secondOrder.sizeDelta != 0 && firstOrder.sizeDelta == -secondOrder.sizeDelta) {
            ...
            if (shareRatioD18 == 0) {
                revert UnauthorizedRelayer(ERC2771Context._msgSender());
            }

            SettlementStrategy.Data storage strategy = PerpsMarketConfiguration
                .loadValidSettlementStrategy(firstOrder.marketId, firstOrder.settlementStrategyId);

            AsyncOrder.Data storage order = AsyncOrder.load(firstOrder.accountId);
