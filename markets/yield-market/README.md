...
Yield Market lets a whitelisted market borrow USD from Synthetix and deploy it into external yield strategies, then repay and optionally deposit acquired collateral back to Synthetix. It supports two borrow modes:

- **Standard**: use `MarketManager.withdrawMarketUsd` / `depositMarketUsd`.
- **Unsecured**: use core `UnsecuredCreditModule.borrowUnsecured` / `repayUnsecured`.

## Core modules

- `YieldMarketFactoryModule`: sets Synthetix refs, min credit, and configures unsecured mode (module address + toggle).
- `ProfitShareModule`: owner-only surface to borrow/repay USD (standard or unsecured), withdraw USD to strategies, and deposit strategy-acquired collateral into core.

## Key flows

- **Borrow USD**: `borrowUsd(to, amount)` → standard or unsecured path, controlled by factory flag.
...
- **Unsecured toggle**: `setUnsecuredConfig(unsecuredModule, useUnsecured)` on the factory to switch modes.

## Deployment (cannon)

- See `cannonfile.toml` for registration, router wiring, and feature-flag allowlisting.
- Workspace build: `yarn workspace @synthetixio/yield-market build`.

## Notes

- Only the market owner (proxy owner) can call the admin/strategy flows.
- Strategy adapters are external; this package only handles borrow/repay and accounting hooks.
- Ensure feature flags `borrowUnsecured`/`repayUnsecured` are enabled in core when using unsecured mode.
